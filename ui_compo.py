...
class VolunteerCard(tk.Frame):
    ...
    def build(self):
        ...
        actions = tk.Frame(top, bg=t["card_bg"])
        ...
        tk.Label(bottom, text=f"📅  {self.data['availability']}",
                 bg=t["card_bg"], fg=t["fg"]).pack(side="left")
        tk.Label(bottom, text=f"🔱  {self.data['experience']}",
                 bg=t["card_bg"], fg=t["fg"]).pack(side="left", padx=28)

        # ---------- Message ----------
        msg_frame = tk.Frame(outer, bg=t["card_bg"])
        msg_frame.pack(fill="x", pady=(8, 6))

        tk.Label(msg_frame, text="Notes:", bg=t["card_bg"], fg=t["fg"]).pack(anchor="w")
        tk.Label(
            msg_frame,
...
class EditDialog(tk.Toplevel):
    def __init__(self, parent, data, on_save, theme_getter,
                 availability_options=None, experience_options=None):
        super().__init__(parent)
        self.data = dict(data)
    ...
    def build(self):
        ...
        fields = [
            ...
            ("Registered:", self.reg_var),
        ]

        for i, (label, var) in enumerate(fields):
            tk.Label(frm, text=label, bg=t["bg"], fg=t["fg"]).grid(row=i, column=0, sticky="w", pady=4)
            tk.Entry(frm, textvariable=var, width=40).grid(row=i, column=1, sticky="w", pady=4)

        # ---------- Dropdowns ----------
        tk.Label(frm, text="Availability:", bg=t["bg"], fg=t["fg"]).grid(row=4, column=0, sticky="w", pady=4)
        ttk.Combobox(frm, textvariable=self.avail_var,
                     values=self.availability_options,
                     state="readonly", width=38).grid(row=4, column=1, sticky="w", pady=4)

        tk.Label(frm, text="Experience:", bg=t["bg"], fg=t["fg"]).grid(row=5, column=0, sticky="w", pady=4)
        ttk.Combobox(frm, textvariable=self.exp_var,
                     values=self.experience_options,
                     state="readonly", width=38).grid(row=5, column=1, sticky="w", pady=4)

        # ---------- Message ----------
        tk.Label(frm, text="Notes:", bg=t["bg"], fg=t["fg"]).grid(row=6, column=0, sticky="nw")
    ...
    def render_tags(self):
        ...
        for tag in self.interests:
            ...
            tk.Button(
                ...
                fg="#F87171",
                bd=0,
                command=lambda x=tag: self.remove_tag(x)
            ).pack(side="left")
